...
func NewPubSubClientWithRetries(ctx context.Context, credentials []byte, retries []time.Duration, projectId string) (*pubsub.Client, error) {
...
func NewPubSubClient(ctx context.Context, credentials []byte, projectId string) (*pubsub.Client, error) {
	opts := option.WithCredentialsJSON(credentials)
	if credentials != nil && len(credentials) > 0 {
